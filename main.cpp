...

{
	...
	for (int i = 0; i < 3; i++) temp.Score[i] = score[i];
	for (auto &i : student) {
		if (i.Num == temp.Num) {
			i = temp;
			return;
...
void Modify() {
	...
	for (int i = 0; i < student.size(); i++) {
		if (student[i].Num == temp.Num) { student[i] = temp; }
	}
}
void Delete() {
	string Num;
	cin >> Num;
	for (int i = 0; i < student.size(); i++) {
		if (student[i].Num == Num) { student.erase(student.begin() + i); }
	}
}
void SearchNum() {
	string Num;
	cin >> Num;
	for (int i = 0; i < student.size(); i++) {
		if (student[i].Num == Num) { cout << student[i].All << endl; }
	}
}
bool CompareNum(Student a, Student b) { return a.Num < b.Num; }
void SearchName() {
	string a, b, Name;
	cin >> a >> b;
	Name = a + " " + b;
	...
	temp.clear();
	for (int i = 0; i < student.size(); i++) {
		if (student[i].Name == Name) { temp.push_back(student[i]); }
	}
	sort(temp.begin(), temp.end(), CompareNum);
	for (int i = 0; i < temp.size(); i++) { cout << temp[i].All << endl; }
}
void CompareNumMain() {
	sort(student.begin(), student.end(), CompareNum);
	for (int i = 0; i < student.size(); i++) cout << student[i].All << endl;
}
